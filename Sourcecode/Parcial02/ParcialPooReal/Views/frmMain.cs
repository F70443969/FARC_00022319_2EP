    ...
    public partial class frmMain : Form
    {
        ...
        private void btnRegister_Click_1(object sender, EventArgs e)
        {
            if (allFieldsAreNotEmpty())
            {
                try
                {
                    DBuser.createNewUser(txtName.Text, txtUsername.Text, txtUsername.Text, chAdmin.Checked);
        ...
        private void btnBusiAction_Click(object sender, EventArgs e)
        {
            if (txtBusiName.Text.Length != 0 && txtBusiDesc.Text.Length != 0)
            {
                if (!isBusiSelected)
                {
                    ...
                    DBbusiness.createBusiness(business);
                    refreshBusinessFields();
                    fillControls();
                    mostrarMensajeExito();
                }
